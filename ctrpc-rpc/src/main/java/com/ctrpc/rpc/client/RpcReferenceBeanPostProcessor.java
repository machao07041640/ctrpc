...
public class RpcReferenceBeanPostProcessor implements BeanPostProcessor {
    ...
    private final ObjectProvider<RpcSerializer> serializerProvider;
    ...
    public Object postProcessBeforeInitialization(Object bean, String beanName) throws BeansException {
        ...
        ReflectionUtils.doWithFields(clazz, field -> {
            ...
            injectReference(bean, field, reference);
        });
        return bean;
    }
    ...
    private void injectReference(Object bean, Field field, RpcReference reference) {
        ...
        if (!iface.isInterface()) {
            throw new IllegalStateException("@RpcReference field must be an interface: " + field);
        }
        ...
        Object proxy = Proxy.newProxyInstance(
                ...
                new RpcInvocationHandler(
                        ...
                        iface,
                        reference.timeoutMs(),
                        channelManagerProvider.getObject(),
                        serializerProvider.getObject(),
                        properties
                )
        );
    ...
    private String resolveServiceName(Class<?> iface, RpcReference reference) {
        ...
        for (Map.Entry<String, RpcProperties.ServiceDependency> entry : properties.getDependencies().entrySet()) {
            if (entry.getValue().getInterfaces() != null
                    && entry.getValue().getInterfaces().contains(ifaceName)) {
                return entry.getKey();
            }
        }
    ...
    private void ensureConfigured(String serviceName, Class<?> iface) {
        ...
        if (dep == null) {
            throw new IllegalStateException("Missing ctrpc.rpc.dependencies." + serviceName);
        }
        if (dep.getInterfaces() != null && !dep.getInterfaces().isEmpty()
                && !dep.getInterfaces().contains(iface.getName())) {
